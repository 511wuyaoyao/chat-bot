/**
 * OneBot11 消息段工具。
 * 统一解析 OneBot 消息段，并提供 @ 自身检测与开头 @ 清理。
 */

import { OneBotMessageSegment } from "../../adapter/protocol/onebot11";
...
export function normalizeMessageSegments(rawSegments: unknown): OneBotMessageSegment[] {
  ...
  return rawSegments
    .filter((segment): segment is Record<string, unknown> =>
      Boolean(segment) && typeof segment === "object" && typeof segment.type === "string"
    )
    .map((segment) => ({
      type: segment.type,
...
function getLeadingSelfMentionNames(rawSegments: unknown, selfId: string): string[] {
  ...
  for (const segment of normalizeMessageSegments(rawSegments)) {
    ...
    if (segment.type === "text" && !String(data?.text ?? "").trim()) continue;
    if (segment.type !== "at" || !isSelfAtValue(data?.qq, selfId)) break;
